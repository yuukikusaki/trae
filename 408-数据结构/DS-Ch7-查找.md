...
| 题目 | 难度 | 推荐理由 | 408考点关联 |
|------|------|----------|------------|
| [704. 二分查找](https://leetcode.cn/problems/binary-search/) | 简单 | 二分查找最标准实现 | 与408 BinarySearch 几乎一致 |
| [35. 搜索插入位置](https://leetcode.cn/problems/search-insert-position/) | 简单 | 二分查找变体 | 408中"查找或插入位置"的判定 |
| [34. 在排序数组中查找元素的第一个和最后一个位置](https://leetcode.cn/problems/find-first-and-last-position-of-element-in-sorted-array/) | 中等 | **二分边界处理** | 408中二分查找的精确应用 |
| [33. 搜索旋转排序数组](https://leetcode.cn/problems/search-in-rotated-sorted-array/) | 中等 | 二分变体 | 408不直接考，但考察对二分本质的理解 |
| [1. 两数之和](https://leetcode.cn/problems/two-sum/) | 简单 | 哈希表O(1)查找 | 哈希表查找的经典应用 |
| [217. 存在重复元素](https://leetcode.cn/problems/contains-duplicate/) | 简单 | 哈希表/排序 | 哈希表判断重复 |
| [349. 两个数组的交集](https://leetcode.cn/problems/intersection-of-two-arrays/) | 简单 | 哈希表集合操作 | 哈希表综合应用 |

### 刷题建议

> 704和34必做，理解二分查找的**边界条件**（while(lo<=hi) vs while(lo<hi)、mid±1的处理）。这两道题做熟后，二分查找的代码不会写错。
...
```c
// 写法1：闭区间 [lo, hi] ← 408最常用
int binarySearch_1(int a[], int n, int key) {
    int lo = 0, hi = n - 1;
    while (lo <= hi) {                 // 闭区间用 <=
        int mid = lo + (hi - lo) / 2;
...
// 写法2：开区间 [lo, hi)
int binarySearch_2(int a[], int n, int key) {
    int lo = 0, hi = n;
    while (lo < hi) {                  // 开区间用 <
        int mid = lo + (hi - lo) / 2;
        if (a[mid] == key) return mid;
...
分块查找的平均查找长度为**索引查找与块内查找之和**：
- 设将长度为 n 的表均匀分成 b 块，每块有 s 个记录
- 索引表顺序查找：ASL_索引 = (b+1)/2
- 块内顺序查找：ASL_块内 = (s+1)/2
- 总：ASL = (b+1)/2 + (s+1)/2 = (b+s+2)/2
- **最优情况**：b=s=√n，ASL_min = √n + 1

---

## 教材补充：B树的高度范围

对于一棵 n 个关键字、m 阶的 B 树：
- **最小高度**（每个结点关键字最多）：$h \le \log_m(\frac{n}{m-1}+1)$
- **最大高度**（每个结点关键字最少）：$h \ge \log_{\lceil m/2 \rceil}(\frac{n}{2}+1)$

> 408常通过B树高度范围来考查对B树性质的理解。
...
以表长 m=13、H(key)=key%13 为例（笔记中的数据）：

每个地址的查找失败比较次数（查到空位为止，空位也算一次）：
...
> **注意**：不同教材对"空地址比较次数"的算法可能有差异。408以实际考试答案为准。这里的计算假设空地址比较1次即失败。

### 链地址法ASL

| 度量 | 公式 |
|------|------|
| 成功ASL | $1 + \frac{\alpha}{2}$ |
| 失败ASL | $\alpha + e^{-\alpha}$ |
...
| 2016 | 综合题 | B树删除与合并 |
| 2018 | 选择题 | 哈希表线性探测 |
| 2020 | 选择题 | 装填因子对ASL的影响 |
| 2021 | 选择题 | B树的概念 |
| 2022 | 选择题 | 哈希表ASL计算 |

> 408规律：查找这一章**几乎只考选择题**（B树偶尔在综合题中出现），侧重于ASL手算和B树/B+树的概念题。哈希表的ASL手算几乎是每年的必考选择题。

---

> 上一章：[[DS-Ch6-图]] | 下一章：[[DS-Ch8-排序]] | [[DS-大题模板]] | [[DS-MOC-数据结构总览|MOC]]
